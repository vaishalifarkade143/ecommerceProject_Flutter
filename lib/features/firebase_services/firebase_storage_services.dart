...
//       final imageData = byteData.buffer.asUint16List(
...
class TFirebaseStorageService extends GetxController {
  ...
  // ✅ Uint8List — correct type for raw image bytes
  Future<Uint8List> getImageDateFromAssets(String path) async {
    try {
      final byteData = await rootBundle.load(path);
      return byteData.buffer.asUint8List(  // ✅ asUint8List not asUint16List
        byteData.offsetInBytes,
        byteData.lengthInBytes,
      );
    } catch (e) {
  ...
  // ✅ Uint8List parameter — no cast needed
  Future<String> uploadImageData(String path, Uint8List image, String name) async {
    try {
      final ref = _firebaseStorage.ref(path).child(name);
      await ref.putData(image);  // ✅ no cast
      final url = await ref.getDownloadURL();
      return url;
    } catch (e) {
      if (e is FirebaseException) {

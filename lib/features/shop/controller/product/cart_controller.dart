...
class CartController extends GetxController {
  ...
  // Encodes to a JSON string — GetStorage cannot reliably serialize
  // nested Map<String,String> (selectedVariation) as a raw List.
  void saveCartItems() {
    final cartItemStrings =
  ...
  // FIX: Read back as String (matches what saveCartItems now writes),
  // then jsonDecode into a List. Old code read as List<dynamic> which
  // mismatched the String written above — causing the malformed JSON crash.
  // The try/catch clears any corrupt legacy data so the crash never repeats.
  void loadCartItems() {
    try {
      final raw = TLocalStorage.instance().readData<String>('cartItems');
      ...
      // Corrupt or legacy data — wipe it so the app never crashes here again
      TLocalStorage.instance().removeData('cartItems');
      cartItems.clear();
    }
  }
